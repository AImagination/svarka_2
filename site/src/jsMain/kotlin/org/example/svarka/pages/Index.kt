...
//                }
        ...
        when {
            ...
            depthValue < 100 -> {
                Dialog("Совпадает ли направление распространения дефекта с направлением излучения просвечивания?", isDirectionTheSame)
                isDirectionTheSame.value?.let {
                    if (it) UltraTipBox(true)
                    else RadicTipBox()
                }
            }
            depthValue < 250 -> {
...
fun InductTipBox() {
    ...
    TipBox("Для выбранной толщины подходит индукционный метод неразрушающего контроля, основным принципом которого является формирование электродвижущей силы с применением индукционных катушек.\n" +
...
fun MagnetTipBox() {
    ...
    TipBox("Для выбранной толщины подходит магнитопорошковый метод неразрушающего контроля, основанный на явлении притяжения частиц магнитного порошка магнитными потоками рассеяния, \n" +
...
fun RadicTipBox() {
    ...
    TipBox("Для выбранной толщины и направления распространения дефекта подходит следующий метод: радиационный (радиографический) контроль (РК) – метод неразрушающего контроля, который позволяет выявить скрытые дефекты сварных стыков без их повреждения. \n" +
...
fun UltraTipBox(
    ...
    val prefix = if(isDirectionUsed) {
        "Для выбранной толщины и направления распространения дефекта подходит следующий метод: "
    } else {
        "Для выбранной толщины подходит "
    }

    TipBox("$prefix ультразвуковой метод контроля сварного шва - метод неразрушающего контроля, позволяющего выявлять скрытые повреждения посредством ультразвука.\n" +
...
fun EndoTipBox() {
    ...
    TipBox("Для выбранной толщины подойдет эндоскоп (технический эндоскоп или бороскоп) – оптический прибор, который используется для визуального осмотра внутренних поверхностей трубок, отверстий или других труднодоступных мест. \n" +
